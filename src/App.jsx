...
const TICKERS = [
  "RDDT","IONQ","SOUN","BBAI","GTLB","CFLT","DDOG","SMAR","MGNI",
  ...
  "CART","SG","XPOF",
  "MARA","RIOT","CLSK","CORZ",
  "GENI","SKLZ","MP","OPEN","LAUR","KPLT","ACMR",
];

// All calls go through /api/fmp proxy → financialmodelingprep.com/stable/
...
function scoreStock(s) {
  ...
  const ps = s.priceToSalesRatio, pe = s.pe;
  const rev = s.revenueGrowth, cap = s.marketCap;
  ...
  if (!cap || cap < 300e6) discovery += 15;
  else if (cap < 1e9) discovery += 11;
  else if (cap < 2e9) discovery += 7;
  else discovery += 3;
...
function Badge({ label, color, bg }) {
  return (
    <span style={{ display:"inline-flex", alignItems:"center", padding:"2px 8px", borderRadius:20, fontSize:10, fontWeight:700, color, background:bg, marginRight:4, marginBottom:4, border:`1px solid ${color}33` }}>
      {label}
    </span>
  );
}
...
export default function LedgerScan() {
  const [filters, setFilters]     = useState({ sector:"all", capFilter:"all", minScore:0 });
  ...
  const runScan = useCallback(async () => {
    ...
    try {
      setLoadMsg(`Fetching live profiles for ${TICKERS.length} stocks...`);

      // Fetch profiles in batches of 10 (stable/profile supports comma-separated symbols)
      const batches = [];
      ...
      const profileMap = {};
      batchResults.flat().forEach(p => { if (p?.symbol) profileMap[p.symbol] = p; });
      ...
      let stocks = TICKERS.map(ticker => {
        ...
        const stock = {
          ...
          pe:                 p.pe || null,
          revenueGrowth:      p.revenueGrowth || null,
        };
        return { ...stock, ...scoreStock(stock) };
      }).filter(Boolean);

      if (filters.sector !== "all")
        stocks = stocks.filter(s => (s.sector||"").toLowerCase().includes(filters.sector.toLowerCase()));
      ...
      stocks = stocks.filter(s => s.total >= filters.minScore && s.price > 0)
                     .sort((a, b) => b.total - a.total);

      if (!stocks.length) throw new Error("No results matched your filters — try broadening them.");
      setResults(stocks);
      setScanned(true);
...
Revenue Growth: ${fmtPct(stock.revenueGrowth)} | P/S: ${fmtNum(stock.priceToSalesRatio)} | P/E: ${fmtNum(stock.pe)}
Beta: ${stock.beta ?? "Unknown"}
${stock.description ? "Description: " + stock.description.slice(0,300) : ""}

{
  "plainEnglishSummary": "2-3 sentences, simple, what they do and why interesting",
    ...
    <div style={{ minHeight:"100vh", background:"#f0f2f7", fontFamily:"'DM Sans',sans-serif" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .filter-select{border:1.5px solid #dde2ed;border-radius:10px;padding:9px 12px;font-family:'DM Sans',sans-serif;font-size:13px;color:#374151;background:white;outline:none;cursor:pointer;}
        ...
        .pill{display:inline-flex;align-items:center;gap:5px;padding:6px 14px;border-radius:20px;border:1.5px solid #e2e6ef;background:white;font-size:11px;font-weight:500;color:#6b7280;cursor:pointer;transition:all 0.2s;white-space:nowrap;}
      ...
      <div style={{ background:"#0f0f1a", borderBottom:"1px solid #1e2235", padding:"16px 28px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          ...
          <div style={{ fontSize:11, color:"#4b5280", marginTop:1, fontFamily:"'DM Mono',monospace" }}>
            LIVE · {TICKERS.length} growth stocks · powered by FMP
      ...
      <div style={{ maxWidth:960, margin:"0 auto", padding:"24px 18px" }}>
          ...
          <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"flex-end" }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
              ...
              <select className="filter-select" value={filters.capFilter} onChange={e=>setFilters(p=>({...p,capFilter:e.target.value}))}>
                ...
                <option value="micro">Tiny (&lt;$300M)</option>
        ...
        {error && <div style={{ background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.3)", borderRadius:12, padding:"12px 16px", marginBottom:16, fontSize:13, color:"#dc2626" }}>⚠ {error}</div>}
        ...
        {!loading && scanned && (
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
            ...
            <span style={{ fontSize:11, color:"#6b7280", background:"#f0f9ff", border:"1px solid #bae6fd", borderRadius:8, padding:"5px 12px" }}>💡 Click any row for AI breakdown</span>
          </div>
        )}
          ...
          <div style={{ background:"linear-gradient(135deg,#f0f0ff,#faf0ff)", border:"1.5px solid #c4b5fd", borderRadius:14, padding:"14px 20px", marginBottom:14, display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:12, color:"#6b7280", marginTop:2 }}>
                ...
                {topPick.revenueGrowth > 0 && ` Revenue up ${fmtPct(topPick.revenueGrowth)} YoY.`}
                {topPick.priceToSalesRatio && topPick.priceToSalesRatio < 5 && ` P/S of ${topPick.priceToSalesRatio.toFixed(1)}x.`}
              </div>
            </div>
          </div>
        )}
              ...
              <div className="row-main" onClick={() => analyseStock(stock)}>
                ...
                <div style={{ minWidth:0 }}>
                  ...
                  <div style={{ marginTop:4, display:"flex", flexWrap:"wrap" }}>
                    ...
                    {stock.revenueGrowth > 0.4 && <Badge label={`🚀 ${fmtPct(stock.revenueGrowth)} rev`} color="#22c55e" bg="rgba(34,197,94,0.07)" />}
                  </div>
                ...
                <div className="hide-mobile" style={{ display:"flex", gap:8 }}>
                  ...
                  <div className="metric-chip">
                    <span style={{ fontSize:14, fontWeight:700, color:stock.revenueGrowth>0.3?"#22c55e":stock.revenueGrowth<0?"#ef4444":"#f59e0b" }}>{fmtPct(stock.revenueGrowth)}</span>
                  ...
                  <div className="metric-chip">
                    <span style={{ fontSize:14, fontWeight:700, color:"#374151" }}>{fmtNum(stock.pe)}</span>
        ...
        {!scanned && !loading && (
          <div style={{ textAlign:"center", padding:"70px 20px" }}>
            ...
            <div style={{ fontSize:13, color:"#9ca3af", maxWidth:420, margin:"0 auto 22px", lineHeight:1.6 }}>
              LedgerScan tracks {TICKERS.length} hand-picked growth companies with real prices and fundamentals from Financial Modeling Prep.
            </div>
        ...
        <div style={{ marginTop:36, fontSize:11, color:"#9ca3af", lineHeight:1.7, padding:"14px 16px", background:"white", border:"1px solid #e5e9f0", borderRadius:12 }}>
          <strong style={{ color:"#6b7280" }}>⚠ Not financial advice.</strong> LedgerScan is an educational tool. Market data sourced from Financial Modeling Prep. Scores are algorithmic and do not constitute investment recommendations. Always do your own research and consult a financial advisor. Investing carries risk.
        </div>
      </div>
    </div>
  );
